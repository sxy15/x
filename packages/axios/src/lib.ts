...
export async function fetch<R>(
  ...
  try {
    const data = handleResponseCommon(await axios.request<ResponseData<R>>({ ...axiosConfig, url, method }), {
...
function isResponseException(
  error: any,
): error is AxiosResponse<{ retcode: number; message: string }> & { [needAttention]?: true } {
...
export function handleResponseCommon<T>(
  response: Promise<AxiosResponse<ResponseData<T>>>,
  options?: { showError?: boolean },
): Promise<T>
export function handleResponseCommon<T>(
  responseOrPromise: MaybePromise<AxiosResponse<ResponseData<T>>>,
  { showError = false } = {},
): T | Promise<T> {
