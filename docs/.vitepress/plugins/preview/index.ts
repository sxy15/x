...
export function vuePreviewPlugin(options: PreviewOptions = {}): Plugin {
  ...
  return {
    ...
    async load(id) {
       ...
       *  base
       *  :::
       *  如何写一个正则匹配出 title: 基础用法  component: base？
       */
      ...
      try {
        ...
        while ((match = (componentREG.exec(code))) !== null) {
          ...
          components.push(`<preview-wrapper title="${title}" source="${encodeSource(source)}" hlSource="${encodeSource(hlSource)}"><${name} /></preview-wrapper>`)
        }
